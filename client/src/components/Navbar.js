...
function Navbar({toggleModal, totalContacts, handleSearch}) {
  ...
  return (
    <section className='navbar-container flex flex-row justify-between'>
        ...
        <div className='searchBar w-7/12 h-full'> <input id="search-input" className='search-input  p-2 w-full border border-1 border-[#cacaca] border-opacity-2 rounded-md'  type='text' placeholder={`Search in ${totalContacts} Contacts`} value={searchString} onChange={handleChange}/></div>
